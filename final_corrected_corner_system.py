...
def get_live_corner_opportunities():
    """Get corner betting opportunities from truly live matches with correct timing"""
    
    api_key = os.getenv('SPORTMONKS_API_KEY')
    
    print("🎯 LIVE CORNER BETTING OPPORTUNITIES (bet365 + CORRECT TIMING)")
    print("=" * 80)
    ...
    # Filter for truly live matches with correct timing
    ...
    if opportunities:
        ...
        for i, opp in enumerate(opportunities):
            ...
            # Enhanced recommendations based on minute
            if opp['minute'] >= 85:
...
def filter_truly_live_matches(matches):
    """Filter matches to only include those truly live with correct timing"""
    ...
    for match in matches:
        ...
        # Check if match is in playing state and has ticking period
        if state_short in ['1H', '1st', '2H', '2nd']:
            actual_minute = get_correct_match_minute(match)
            
            if actual_minute > 0:  # Only include if we have a valid minute
...
def analyze_live_match_for_corners(match, api_key):
    """Analyze a live match for corner betting opportunity with correct timing"""
    ...
    # Calculate psychology score with CORRECT minute
    ...
    return {
        ...
        'league': league_name,
        'scoreline': scoreline,
        'minute': live_minute,  # CORRECT minute from ticking period
        'state': live_state,
        'home_odds': home_odds,
        'away_odds': away_odds,
        'draw_odds': draw_odds,
...
def calculate_psychology_score(home_score, away_score, favorite, confidence, minute):
    """Calculate psychology score based on scoreline, favorite status, and CORRECT match time"""
    
    psychology_score = 0
    goal_diff = abs(home_score - away_score)
    
    # Time-based multiplier using CORRECT minutes
    if minute >= 85:
        ...
        time_multiplier = 1.0  # Regular time
