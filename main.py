...
class LateCornerMonitor:
    ...
    async def _monitor_single_match(self, match_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            self.logger.info(f"   📊 Minute: {match_stats.minute} (need 74-78)")
            ...
            # 🚨 MANDATORY TIMING CHECK: Only proceed with alert analysis if in 74-78 minute window (MOMENTUM INVERTED)
            if not (74 <= match_stats.minute <= 78):
                self.logger.info(f"⏰ TIMING CHECK FAILED: Match at {match_stats.minute}' (need 74-78 minutes) - SKIPPING ALERT ANALYSIS")
                # Update previous stats for momentum tracking on next cycle
                self.previous_stats[fixture_id] = copy.deepcopy(current_stats)
                return None
            
            self.logger.info(f"✅ TIMING CHECK PASSED: Match at {match_stats.minute}' (within 74-78 minute window)")
            ...
            timing_ok = 74 <= match_stats.minute <= 78
            odds_ok = current_stats.get('has_live_asian_corners', False)
            
            self.logger.info(f"   ⏱️ Timing (74-78min): {'✅ OK' if timing_ok else '❌ FAIL'} (minute {match_stats.minute})")
